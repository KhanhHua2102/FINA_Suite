...
# Kraken SDK imports - no longer needed, using direct API calls
# from kraken.spot import Market as KrakenMarket
# from kraken.spot import SpotClient

# -----------------------------
# Kraken market-data (current ASK):
#   Uses python-kraken-sdk to get ticker information
#   Returns ask price from ticker data
# -----------------------------

# Symbol mapping from original format to Kraken format
SYMBOL_MAPPING = {
    "BTC-USD": "XXBTZUSD",
    "ETH-USD": "XETHZUSD",
    "XRP-USD": "XXRPZUSD",
...
def _get_kraken_symbol(symbol: str) -> str:
    """Convert symbol from BTC-USD format to Kraken format like XXBTZUSD"""
...
class KrakenMarketData:
    def __init__(self):
        # No longer using Kraken SDK - direct API call instead
        pass

    def get_current_ask(self, symbol: str) -> float:
        """Get current ask price from Kraken ticker data"""
        kraken_symbol = _get_kraken_symbol(symbol)

        try:
            # Get ticker information
            ticker_data = self.market.get_ticker_information(kraken_symbol)

            # ticker_data is a pandas DataFrame, get the ask price
            # The 'a' column contains ask array [price, whole_lot_volume, lot_volume]
            if kraken_symbol in ticker_data.index:
                ask_data = ticker_data.loc[kraken_symbol, "a"]
                # ask_data is a list/array, first element is the price
                if isinstance(ask_data, (list, tuple)) and len(ask_data) > 0:
                    return float(ask_data[0])
                else:
                    return float(ask_data)
            else:
                raise RuntimeError(
                    f"Kraken symbol {kraken_symbol} not found in ticker data"
                )

        except Exception as e:
            raise RuntimeError(
                f"Failed to get Kraken ticker for {symbol} ({kraken_symbol}): {e}"
            )
...
def kraken_current_ask(symbol: str) -> float:
    ...
    Uses public Kraken API - no credentials needed.
    """
    kraken_symbol = _get_kraken_symbol(symbol)

    try:
        # Use Kraken public API directly
        url = f"https://api.kraken.com/0/public/Ticker?pair={kraken_symbol}"
        response = requests.get(url, timeout=10)

        if response.status_code != 200:
            raise RuntimeError(
                f"Kraken API returned status {response.status_code}: {response.text}"
            )
        ...
        if "result" not in data:
            raise RuntimeError(f"Kraken API response missing 'result': {data}")

        if kraken_symbol not in data["result"]:
            raise RuntimeError(
                f"Kraken symbol {kraken_symbol} not found in response: {list(data['result'].keys())}"
            )

        ticker = data["result"][kraken_symbol]
        ask_data = ticker.get("a", [])

        if not ask_data or len(ask_data) == 0:
            raise RuntimeError(
                f"Kraken ticker missing ask data for {kraken_symbol}: {ticker}"
            )

        # Ask data is [price, whole_lot_volume, lot_volume]
...
def step_coin(sym: str):
    ...
    if tf_choice_index >= len(tf_choices):
        ...
        # get current price ONCE per coin — use Kraken's current ASK
